...
const float locPI = 3.14159265359f;
const float locDegToRad = 180 / locPI;
...
namespace CommonUtilities
{
	...
	{
		if (aValue < aMax && aValue > aMin) return aValue;
	...
	int GetAngleBetweenVectors(CU::Vector2<float> aFirst, CU::Vector2<float> aSecond)
	{
		...
		return int(angle * locDegToRad + 0.5f);
	}

	float FacePoint(const CU::Vector2<float>& aOrigin, const CU::Vector2<float>& aTargetPosition)
	{
		float dX = aTargetPosition.x - aOrigin.x;
		float dY = aTargetPosition.y - aOrigin.y;
		return (atan2(dY, dX));
	}
